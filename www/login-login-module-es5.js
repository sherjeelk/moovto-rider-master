...
var LoginPage = /** @class */ (function () {
    function LoginPage(http, router, storage, toast, socket) {
        this.http = http;
        this.router = router;
    ...
    LoginPage.prototype.login = function (username, pass) {
        ...
        this.http.post(_AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].API.LOGIN, body).subscribe(function (res) {
            var result = JSON.parse(JSON.stringify(res));
            console.log('Result', result);
            if (result.user.isRider) {
                _this.storage.set('user', result.user);
                _this.storage.set('token', result.jwt);
                _this.storage.set('login', true);
                ...
                if (navigator) {
                    if (_AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].IS_LOGGED_IN) {
                        _this.storage.get('user').then(function (value) {
                            ...
                            console.log(value);
                            _this.socket.emit('riders', { action: 'add', rider: _AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].user });
                ...
                _this.presentToast('You are not a rider!');
            }
        }, function (error) {
            console.error(error);
        });
    ...
    LoginPage.prototype.updateLocation = function () {
        var _this = this;
        this.http.put(_AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].API.RIDERS + '/' + _AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].user.rider, { lat: _AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].lat, lng: _AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].lng }, { headers: { Authorization: "Bearer " + _AppConstants__WEBPACK_IMPORTED_MODULE_4__["AppConstants"].token } }).subscribe(function (value) {
            ...
            _this.socket.emit('riders', { action: 'update', rider: JSON.parse(JSON.stringify(value)) });
        }, function (error) {
