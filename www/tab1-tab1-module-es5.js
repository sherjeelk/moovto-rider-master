...
var Tab1Page = /** @class */ (function () {
    function Tab1Page(storage, router, socket, socketService, http) {
        var _this = this;
        ...
        this.storage.get('order').then(function (value) {
            if (value !== undefined && value !== null) {
                _this.getOrderDetails(value);
            }
        });
        this.storage.get('status').then(function (value) {
    ...
    Tab1Page.prototype.action = function (id, accept) {
        if (accept) {
    ...
    Tab1Page.prototype.notifyLocation = function () {
        var _this = this;
        if (navigator) {
            if (_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].IS_LOGGED_IN) {
                this.storage.get('user').then(function (value) {
                    ...
                    navigator.geolocation.getCurrentPosition(function (pos) {
                        ...
                        _this.lat = _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].lat;
                        _this.lng = _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].lng;
        ...
        setTimeout(function () {
            _this.notifyLocation();
        }, 30000);
    };
    Tab1Page.prototype.updateLocation = function () {
        var _this = this;
        console.log('About to make call', JSON.stringify(_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.rider));
        var id = _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.rider.id === undefined ? _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.rider : _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.rider.id;
        this.http.put(_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].API.RIDERS + '/' + id, { lat: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].lat, lng: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].lng }, { headers: { Authorization: "Bearer " + _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].token } }).subscribe(function (value) {
            console.log('Update location', JSON.stringify(value));
            _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.rider = value;
            var riderInfo = JSON.parse(JSON.stringify(value));
            ...
            if (!_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].ADDED) {
                _this.socket.emit('riders', { action: 'add', rider: riderInfo });
                _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].ADDED = true;
            }
            console.log('Will update this location', riderInfo);
            _this.socket.emit('riders', { action: 'update', rider: riderInfo });
        }, function (error) {
            console.log('An error occurred!', JSON.stringify(error));
        });
    };
    Tab1Page.prototype.acceptOrder = function (id) {
        var _this = this;
        this.http.put(_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].API.ORDERS + '/' + id, { rider: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.id, status: 'ASSIGNED DELIVERY' }, { headers: { Authorization: "Bearer " + _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].token } }).subscribe(function (data) {
            console.log('Order details', data);
            _this.order = data;
            _this.onGoing = true;
            _this.status = 'pickup';
            _this.storage.set('status', _this.status);
            _this.storage.set('order', id);
        }, function (error) {
            console.log('Looks like an error occurred', error);
        });
    };
    Tab1Page.prototype.getOrderDetails = function (id) {
        var _this = this;
        this.http.get(_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].API.ORDERS + '/' + id).subscribe(function (data) {
            console.log('Order details', data);
            _this.order = data;
            _this.onGoing = true;
            _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].ONGOING = true;
            if (_this.status !== '' && _this.status !== null && _this.status !== undefined) {
    ...
    Tab1Page.prototype.getMapImage = function () {
    ...
    Tab1Page.prototype.convertToCard = function () {
        if (this.map) {
            this.map = false;
        }
    };
    Tab1Page.prototype.openMap = function () {
    ...
    Tab1Page.prototype.markPickupComplete = function () {
        var _this = this;
        this.http.put(_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].API.ORDERS + '/' + this.order.id, { rider: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.id, status: 'PICKUP COMPLETE' }, { headers: { Authorization: "Bearer " + _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].token } }).subscribe(function (data) {
            console.log('Order details', data);
            _this.order = data;
            _this.onGoing = true;
            _this.status = 'drop';
            _this.storage.set('status', _this.status).then(function (value) {
                console.log('Status set');
            });
            _this.storage.set('order', _this.order.id);
            _this.socketService.sendAdminMessage({
                accept: true, title: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.name + " completed pickup of Order# " + _this.order.id + "!",
    ...
    Tab1Page.prototype.markDropComplete = function () {
        var _this = this;
        this.http.put(_AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].API.ORDERS + '/' + this.order.id, { rider: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.id, status: 'DROP COMPLETE' }, { headers: { Authorization: "Bearer " + _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].token } }).subscribe(function (data) {
            console.log('Order details', data);
            _this.order = data;
            _this.onGoing = false;
            _this.status = 'drop';
            _this.storage.remove('status');
            _this.storage.remove('order');
            _this.socketService.sendAdminMessage({
                accept: true, title: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.name + " completed drop of Order# " + _this.order.id + "!",
                avatar: _AppConstants__WEBPACK_IMPORTED_MODULE_6__["AppConstants"].user.avatar
            });
            _this.router.navigateByUrl('/delivery-successful');
        }, function (error) {
            console.log('Looks like an error occurred', error);
        });
    };
